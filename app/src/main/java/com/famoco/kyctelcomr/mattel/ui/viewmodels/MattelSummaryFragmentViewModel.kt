package com.famoco.kyctelcomr.mattel.ui.viewmodels
...
MattelSummaryFragmentViewModel @Inject constructor(private val mainRepository: MainRepository,
    ...
    fun sendData() {
        ...
        val option = (mainRepository.customer.value as com.famoco.kyctelcomr.mattel.model.Customer).option
       val  isFinger= mainRepository.customer.value!!.templates.nbTemplate >=1
        if(option == "2"){
            if (msisdn != null) {
                uploadRepository.sendModificationMattel(msisdn,isFinger)
            }
        }else if (option == "3"){
            if (msisdn != null && imsi != null) {

             //   Toast.makeText()
                Log.d("mattel", "num msisdn"+imsi)

                uploadRepository.sendUSSDMattel(msisdn,imsi,isFinger)

            }
        }
        else if (msisdn != null && imsi != null) {
            Log.d("mattel", "num msisdn"+msisdn)
            uploadRepository.sendSMSMattel(msisdn,imsi,isFinger);
