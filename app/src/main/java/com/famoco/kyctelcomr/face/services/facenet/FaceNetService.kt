...
class FaceNetService @Inject constructor(
    ...
    fun load() {
        val modelFile = MyUtil.loadModelFile(context.assets, "facenet.tflite")
    ...
    fun matchFaces(face1: Bitmap, face2: Bitmap): Float {
        val face1Embeddings = getFaceEmbedding(face1)
        val face2Embeddings = getFaceEmbedding(face2)
        val embeddings = arrayOf(face1Embeddings, face2Embeddings)
        return (cosineSimilarity(embeddings[0], embeddings[1]) + 1) / 2
    }
